...
class GameSetting {
  ...
  onAnswer(user, answerId) {
    ...
    clearTimeout(this.secondQuizTimeout);
  ...
  startReadyTimer() {
    ...
    if (this.mode === 'practice') {
      document.querySelector('.guide').textContent = '練習モード！CPUが不在なのでカードを覚えよう！'
    }
  ...
  speechCard() {
    ...
    this.quizInterval = setInterval(() => {
      ...
      if (char === '　' || char === '、') {
        ...
        clearInterval(this.quizInterval);
        this.secondQuizTimeout = setTimeout(() => {
